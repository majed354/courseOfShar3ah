...
const coveragePath = `${root}/assets/course-specifications/systems-law-completion-20260915/coverage.json`;
...
if (
  ...
  && !errorScan.ndjson.includes('"count":0')
  && !errorScan.ndjson.includes("matched 0 entries")
) {
  throw new Error(`Formula errors found: ${errorScan.ndjson}`);
}

const output = await SpreadsheetFile.exportXlsx(workbook);
await output.save(workbookPath);
const previewDir = `${root}/tmp/systems-law-spreadsheet-update-20260915`;
...
for (const range of [
  "الملخص!A1:I21",
  `المقررات المفقودة!A1:L${identitiesEnd}`,
  `مواضع الخطط!A1:O${occurrencesEnd}`,
]) {
